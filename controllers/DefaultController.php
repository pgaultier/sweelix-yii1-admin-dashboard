...
class DefaultController extends Controller {
	...
	public function actionIndex() {
		...
		if((\Yii::app()->getModule('sweeft')->hasModule('structure') === true) && (\Yii::app()->user->checkAccess('structure') === true)) {
	...
	public function getDatabaseStatus() {
		if($this->_databaseStatus === null) {
			$this->_databaseStatus = array(
				'tables' => array('authors'=>false, 'contentMeta'=>false, 'contentTag'=>false, 'contents'=>false, 'groups'=>false, 'languages'=>false, 'metas'=>false, 'nodeMeta'=>false, 'nodeTag'=>false, 'nodes'=>false, 'tags'=>false, 'templates'=>false, 'urls'=>false),
				'views' => null,
				'procedures' => array('spContentMove'=>false, 'spContentReorder'=>false, 'spFlushUrl'=>false, 'spNodeDelete'=>false, 'spNodeInsert'=>false, 'spNodeMove'=>false),
			);
			$procedures = \Yii::app()->db->createCommand('SHOW PROCEDURE STATUS')->queryAll();
			foreach($procedures as $proc) {
				if(isset($this->_databaseStatus['procedures'][$proc['Name']]) === true) {
					$this->_databaseStatus['procedures'][$proc['Name']] = true;
				}
			}
			$tables = \Yii::app()->db->createCommand('SHOW TABLES')->queryAll(false);
			foreach($tables as $table) {
				if(isset($this->_databaseStatus['tables'][$table[0]]) === true) {
					$this->_databaseStatus['tables'][$table[0]] = true;
				}
			}
		}
		return $this->_databaseStatus;
	}
